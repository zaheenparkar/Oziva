...
{
    public partial class ShipperScanning : Form
    {
        string strFlag = "";
        string strProcess = "";
        ...
        private void txtSecParent_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                lblSecCount.Text = "0";
                string strBarcode = "";
                string strGTIN = "",strpacksize  = "", strBatch = "";
                ParPackLvl = 0;
                lblSecChildLvl.Text = "";
                DataTable dt = new DataTable();
                ...
                try
                {
                    if (txtSecParent.Text != "")
                    {
                        ...
                        strBarcode = txtSecParent.Text;
                        //strGTIN = txtSecParent.Text.Substring(1, 11);

                      //  strpacksize = txtSecParent.Text.Substring(3,3);
                        // strpacksize = txtSecParent.Text.Substring(26);
                        //strBatch = strBatch.Substring(0, strBatch.Length - 15);

                        dt = objScan.BL_GetShipperJOBDt(strBarcode);
                        if (dt.Rows.Count > 0)
                        {

                            strGTIN = dt.Rows[0][1].ToString();

                            string strStatus =
                             objScan.BL_ShipperMapping(cboPacking.Text, "P", strGTIN, strBarcode, strBarcode, PL_Login.UserID.ToString());
        ...
        private void txtSecChild_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                try
                {
                    if ( lblSecGTIN.Text == "" || lblSecPack.Text == "")
                    {
                        DisplayMessage(true, "Scan Parent Barcode", "MAPPING");
                        txtSecParent.Text = "";
                        txtSecParent.Focus();
                    ...
                    strPBarcode = txtSecParent.Text;
                    strCBarcode = txtSecChild.Text;
                   ...
                   // SetPackLevel(lblSecChildLvl, txtSecChild.Text, "C");
                    ...
                    string strStatus = objScan.BL_ShipperMapping(cboPacking.Text, "C", lblSecGTIN.Text, strPBarcode, strCBarcode, PL_Login.UserID.ToString());               
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            if (DialogResult.Yes == ds)
                            {
                                ...
                                strStatus = "";
                                    ...
                                    //= objScan.BL_ManualMapping(ParPackLvl, cboPacking.Text, "P", strGTIN, strBatch, Convert.ToInt32(dt.Rows[0][0].ToString()), strBarcode, "", PL_Login.UserID.ToString(), "1", cboLabelType.Text);
        ...
        private void grpGTIN_Enter(object sender, EventArgs e)
        {
